...
public class SymbolOverview extends JPanel implements BoxFileModelComponent {
	...
	public static final Comparator<Entry<String, List<Symbol>>> SYMBOL_GROUP_COMP = (o1, o2) -> o2.getValue().size()
			- o1.getValue().size();
	...
	public void setBoxFileModel(BoxFileModel model) {
		...
		final JList<Entry<String, List<Symbol>>> glyphList = getSymbolGroupList().getList();

		final HashMap<String, List<Symbol>> glyphs = new HashMap<>();

		final BoxFileModel boxFile = model;
		...
		getSymbolVariantList().getList().setCellRenderer(new SymbolVariantListCellRenderer(boxFile.getImage()));
		...
		final ArrayList<Entry<String, List<Symbol>>> entries = new ArrayList<>(glyphs.entrySet());

		Collections.sort(entries, SYMBOL_GROUP_COMP);

		final DefaultListModel<Entry<String, List<Symbol>>> listModel = new DefaultListModel<>();
