...
public class BatchExportDialog extends JDialog implements ActionListener {
	...
	public BatchExportDialog() {
		...
		panel_3.setBorder(
				new CompoundBorder(
						new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Export Options",
		...
		panel_2.setBorder(
				new CompoundBorder(
						new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Advanced Settings",
								TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)),
						new EmptyBorder(4, 4, 4, 4)));
		panel_1.add(panel_2);
		...
		spinnerWorkerThreads.setModel(new SpinnerNumberModel(DEFAULT_THREADS, MIN_THREADS, MAX_THREADS, 1));
	...
	public void actionPerformed(ActionEvent evt) {
		if (evt.getSource() == btnCancel) {
			...
			try {
				...
				final Path destinationDir = Paths.get(tfDestinationDir.getText());

				if (!Files.exists(destinationDir)) {
					throw new InvalidPathException(destinationDir.toString(), "doesn't exist");
				}
				...
				exportModel = new BatchExportModel(destinationDir, exportTXT, exportHTML, exportXML, exportImages,
						exportReports, numThreads, openDestination);
